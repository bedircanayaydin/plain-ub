...
async def check_xposed_updates(bot: BOT = bot, message: Message = None):
    ...
    if changelog_html:
        ...
        text_parts.append(f"<blockquote expandable=true>{soup.text[0:3000]}</blockquote>\n")
...
async def check_and_insert_to_db(module) -> bool:
    ...
    old_package_name = last_post.get("package_name")
    old_release = last_post.get("last_release")
    ...
    data = dict(
        package_name=package_name, last_release=latest_release, _id="last_updated_post"
    )
