...
async def start_checker():
    ...
    if checker_task is None or checker_task.done():
        checker_task = asyncio.create_task(update_checker_loop())
        print("✅ Magisk checker started automatically")
...
async def show_status(bot: BOT, message: Message):
    status_text = "📊 **Tracked Magisk Modules:**\n\n"
    count = 0
    ...
    if count == 0:
        await message.edit("❌ No modules tracked yet")
        return
...
@bot.add_cmd(cmd="magiskrestart")
async def restart_checker(bot: BOT, message: Message):
    global checker_task
    ...
    await start_checker()
    await message.edit("✅ Magisk checker restarted")
